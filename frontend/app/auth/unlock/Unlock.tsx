...
export default function Unlock({ redirect }: UnlockProps) {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const decResult = await decryptGeneratedKey({
        salt,
        iv,
      ...
      <form onSubmit={handleSubmit} className="md:shadow-slate-500 md:shadow-sm md:rounded-xl">
        <Box className="md:p-10 space-y-2 md:w-[500px]">
            ...
            >
              ...
              <TextField.Slot pr="3">
                <IconButton
                  ...
                  type="button"
                >
                  {visibleKey === "text" ? <IoEyeOff height="16" width="16" /> : <IoEye height="16" width="16" />}
                </IconButton>
              </TextField.Slot>
            </TextField.Root>
          </Box>
          ...
          <div className="mt-5 w-full">
            <Button loading={loading} style={{ width: "100%" }} type="submit" size="3" variant="classic" color="gray">
              Access Vault
            </Button>
          </div>
        </Box>
